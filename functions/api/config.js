...
export async function onRequest(context) {
    try {
        // Cloudflare Pages 설정에서 비밀 변수의 이름이 'VITE_KAKAO_JS_KEY'인지 확인해주세요.
        const kakaoKey = context.env.VITE_KAKAO_JS_KEY;

        if (!kakaoKey) {
            console.error("VITE_KAKAO_JS_KEY secret is not set in Cloudflare environment.");
